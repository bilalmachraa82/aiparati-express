#!/usr/bin/env python3
"""
AutoFund AI - POC Melhorado com Dados Reais
Processa IES PDF → Extração Claude 3.5 → Validação → Análise Opus → Excel IAPMEI

Melhorias baseadas na análise crítica:
- Extração resiliente a quebras de página
- Validação cruzada de subtotais
- Mapeamento Excel por labels (não células fixas)
- Sistema de scoring de risco
- Context handling para análise qualitativa
"""
...
class ExcelGenerator:
    ...
    def find_cell_by_label(self, ws, label: str, search_direction: str = "right") -> Optional[Tuple[int, int]]:
        """Procura por um label na worksheet e retorna célula adjacente"""
                ...
                if cell.value and isinstance(cell.value, str):
                    ...
                    if label_lower in cell_value:
                        ...
                        if search_direction == "right":
                            # Procura próxima célula não vazia à direita
                            for c in range(col_idx + 1, ws.max_column + 1):
                                if ws.cell(row=row_idx, column=c).value is None:
                                    return (row_idx, c)
                        elif search_direction == "down":
                            # Procura próxima célula não vazia abaixo
            ...
            for field, labels in self.field_mappings.items():
                if field in all_data:
                    ...
                    for label in labels:
                        ...
                        if cell_pos:
                            row, col = cell_pos
                            # Verificar se célula não está merged
                            target_cell = ws.cell(row=row, column=col)
                            if not hasattr(target_cell, 'is_merged'):
    ...
    def process_ies(self, pdf_path: str, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "dados_financeiros": financial_data.dict(),
